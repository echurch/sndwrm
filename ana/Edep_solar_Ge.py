...
if __name__=="__main__":
    ...
    fq = TFile("../build/proton_sqbbc_sol.root")
    fs = TFile("../build/proton_shielding_sol.root")

    Npq = fq.T1.GetEntries()
    Nps = fs.T1.GetEntries()
    #Ellen says units on y-axis are EU/day/cm2, with EU being MeV for protons for GCRs
    fsource = TFile("tgraph-spectra_solar/job1c_spectra.root")
    ...
    hs.GetYaxis().SetTitle(f"OLTARIS protons - arb. units")
    ...
    c1.SaveAs("ps_HPGe_OLTARIS1clo_homog_SHIELDING_SPQBBC.png")
    del c1
