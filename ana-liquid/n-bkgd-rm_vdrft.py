...
fidx = 2100 # 0.9 m buffer of LAr on both sides in drift dimension
fidy = 1600 # 0.9 m buffer top/bottom, 0.50 m buffer on each side around central cathode.
sepy = 500
...
def allvertices (x,y,z,t,ID,trkin):
    ...
    for trk in range(-100,100,1):
        ...
        # This does not change with new double fiducialization in Vert Drift detector
...
for trk in range(Nent):
    ...
    fidv = abs(f.Tracks.Startx) < fidx and abs(f.Tracks.Starty) < (fidy+sepy) and abs(f.Tracks.Starty) > sepy and abs(f.Tracks.Startz) < fidz
...
fout = fname.split("/")[-1].split(".")[0] + "_vdrft"
...
gStyle.SetOptStat("")
lineup = TLine(-fidx,fidy+sepy,fidx,fidy+sepy)
linelo = TLine(-fidx,sepy,fidx,sepy)
linel = TLine(-fidx,sepy,-fidx,fidy+sepy)
liner = TLine(fidx,sepy,fidx,fidy+sepy)
...
# And now lower fidv
llineup = TLine(-fidx,-sepy,fidx,-sepy)
llinelo = TLine(-fidx,-fidy-sepy,fidx,-fidy-sepy)
llinel = TLine(-fidx,-sepy,-fidx,-fidy-sepy)
lliner = TLine(fidx,-sepy,fidx,-fidy-sepy)
...
plt.imshow(h2a[0].T,aspect='auto',extent=[-6200,6200,-6200,6200])
#plt.xlim(-6200.,6200.)
#plt.ylim(-6200.,6200.)
...
plt.imshow(h2b[0].T,aspect='auto',extent=[-6200,6200,-32000,32000])
#plt.xlim(-6200.,6200.)
...
plt.imshow(h2c[0].T,aspect='auto',extent=[-6200,6200,-32000,32000])
#plt.xlim(-6200.,6200.)
#plt.ylim(-32000.,32000.)
